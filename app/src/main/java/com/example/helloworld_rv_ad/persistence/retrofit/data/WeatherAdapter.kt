...
import com.example.helloworld_rv_ad.R

class WeatherAdapter(private var weatherList: List<WeatherList>) : RecyclerView.Adapter<WeatherViewHolder>(){
...
override fun onBindViewHolder(holder: WeatherViewHolder, position: Int) {
    ...
    holder.weatherM.text = "${weatherItem.weather?.get(0)?.main?: "ND"} / ${weatherItem.weather?.get(0)?.description?: "ND"}"
    Glide.with(holder.itemView.context).load("http://openweathermap.org/img/wn/${weatherItem.weather?.get(0)?.icon?: "04d.png"}.p")
...
fun updateWeatherData(newWeatherList: List<WeatherList>) {
